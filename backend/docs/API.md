...
`language` is present only when the deployment sets `LANGUAGE_BACKEND=voxlingua_ecapa`. The two negative answers are distinct and stay distinct: an **absent field** means language identification is not configured, and **`unknown`** means it is configured but nothing could be determined. `language.prediction` is an ISO-639 style tag such as `en`, `hi`, or `th`, or `unknown`. It names a spoken language and nothing else — not a locale, accent, dialect, region, or nationality — and the model has no non-speech class, so music or noise can still produce a tag.

Two guards make `unknown` common, deliberately. Audio shorter than `LANGUAGE_MIN_SECONDS` (default 5) never reaches the classifier at all: measured on real English, a three-second window scored Latin at 0.929, and a confidence threshold cannot filter a confident wrong answer. And `LANGUAGE_ALLOWLIST` — strongly recommended — names the languages a deployment serves, removing the other classes from contention; with it set, the acceptance floor is applied to the summed mass of the permitted languages rather than to one class out of 107. Expect no language for the first few seconds of a live session.
