...
ffmpeg -y -i /path/to/your-recording.m4a \
  -ac 1 -ar 16000 -c:a pcm_s16le samples/caller.wav
```

On Linux with ALSA, you can record directly if `arecord` is available:

```bash
arecord -d 5 -t wav -f S16_LE -r 16000 -c 1 samples/caller.wav
```
...
ffprobe -v error \
  ...
  -of default=noprint_wrappers=1 samples/caller.wav
```
...
```bash
ffmpeg -y -i samples/caller.wav \
  -f s16le -ac 1 -ar 16000 samples/caller.s16le
```
...
```bash
ffmpeg -y -i samples/caller.wav \
  -f mulaw -ac 1 -ar 8000 samples/caller.mulaw
